...
class Card:
	def __init__(self, name):
		box = [x for x in range(1, 90)]
		self.card = [
		    __class__.gen_string(box),
		    __class__.gen_string(box),
		    __class__.gen_string(box)
		]
		self.name = name
	...
	def gen_string(box):
		...
		for x in range(8, 3, -1):
			...
			while string[digit] != '':
				digit += 1
			string[digit] = box.pop(randint(0, len(box) - 1))
		return string
...
box = [x for x in range(1, 91)]
while True:
	if len(box) < 1:
		print('Бочёнки закончились. Результат:\n'
		      'у компьютера осталось {} чисел,\n'
		      'у игрока осталось {} чисел.'.format(
		          computer.count_barrel, player.count_barrel))
		break
	barrel = box.pop(randint(0, len(box) - 1))
	print('\nНовый бочонок: {} (осталось {})'.format(barrel, len(box)))
	print(computer)
	print(player)
	...
	while len(reply) == 0 or reply not in 'ynq':
		print('\n\nНезивестный ввод\n')
		print('Новый бочонок: {} (осталось {})'.format(barrel, len(box)))
		print(computer)
		print(player)
		reply = input('Зачеркнуть цифру? (y/n/q)')
		reply = reply.lower()
